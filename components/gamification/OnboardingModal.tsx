...
const LEVELS = [
  { v: 'A1', label: 'A1 — Anfänger', icon: '🌱' },
  { v: 'A2', label: 'A2 — Grundkenntnisse', icon: '📗' },
  { v: 'B1', label: 'B1 — Mittelstufe', icon: '📘' },
  { v: 'B2', label: 'B2 — Fortgeschritten', icon: '🎓' },
];
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 650, background: 'rgba(0,0,0,.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ width: '100%', maxWidth: 460, background: 'var(--bg)', borderRadius: 20, overflow: 'hidden', boxShadow: 'var(--sh-lg)', border: '1px solid var(--border)' }}>
        <div style={{ padding: '20px 24px', background: 'linear-gradient(135deg, #1d4ed8, #4c1d95)', color: '#fff' }}>
          ...
          <div style={{ fontFamily: 'var(--font-lora, Lora), serif', fontSize: 20, fontWeight: 800, marginTop: 4 }}>Willkommen!</div>
          <div style={{ fontSize: 13, opacity: .85 }}>Lass uns deinen Lernplan einrichten — Schritt {step + 1} von 3</div>
        </div>

        <div style={{ padding: 24 }}>
          {step === 0 && (
            <>
              <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--ink)', marginBottom: 12 }}>Welches Niveau hast du?</div>
          ...
          {step === 1 && (
            <>
              <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--ink)', marginBottom: 4 }}>Was interessiert dich?</div>
              <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 12 }}>Wähle beliebig viele (optional)</div>
          ...
          {step === 2 && (
            <>
              <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--ink)', marginBottom: 4 }}>Dein Tagesziel</div>
              <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 12 }}>Wie viele Übungen pro Tag?</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                {GOALS.map(g => (
                  <button key={g} onClick={() => setGoal(g)}
                    ...
                    <div style={{ fontSize: 11, fontWeight: 600 }}>Übungen</div>
                  </button>
                ))}
              </div>
            </>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 22 }}>
            {step > 0 && (
              <button onClick={() => setStep(s => s - 1)} style={{ padding: '11px 18px', borderRadius: 11, border: '1px solid var(--border)', background: 'var(--bg2)', color: 'var(--ink2)', fontWeight: 600, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>← Zurück</button>
            )}
